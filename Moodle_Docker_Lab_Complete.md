...
**บันทึกผลการทดลอง 2:**
 ...
 <img width="881" height="375" alt="image" src="https://github.com/user-attachments/assets/ad37da64-0d34-4138-88db-5c00d303165b" />
...
<img width="1343" height="528" alt="image" src="https://github.com/user-attachments/assets/23a61825-8ba1-4619-8994-b5cb335c0203" />
...
**บันทึกผลการทดลอง 5:**
`
<img width="1027" height="576" alt="image" src="https://github.com/user-attachments/assets/c63482d0-e63f-4f2d-ab9a-865cb050086f" />
...
<img width="1887" height="652" alt="image" src="https://github.com/user-attachments/assets/ae89a2d4-4da8-47ec-8f01-5a42a2da3bcd" />
...
<img width="1915" height="518" alt="image" src="https://github.com/user-attachments/assets/fc376868-92b7-4391-a928-9fc456da5c9e" />
...
<img width="1915" height="761" alt="image" src="https://github.com/user-attachments/assets/fe4bed99-fcc8-4a98-b085-d05ce586c2b6" />
...
คำตอบ:
```
Docker Image คือแม่แบบ (Template) สำหรับสร้าง Container ภายใน Image จะประกอบด้วยระบบปฏิบัติการพื้นฐาน โปรแกรม และการตั้งค่าต่าง ๆ โดย Image จะไม่สามารถทำงานได้เอง

Docker Container คืออินสแตนซ์ที่ถูกสร้างขึ้นจาก Docker Image ซึ่งสามารถรันและใช้งานได้จริง เป็นสภาพแวดล้อมที่แอปพลิเคชันทำงานอยู่
```
...
คำตอบ:
```
1. Web Application Container ทำหน้าที่รันเว็บแอปพลิเคชัน  รับคำสั่งจากผู้ใช้งานผ่านเว็บเบราว์เซอร์ติดต่อกับฐานข้อมูลเพื่อบันทึกหรือดึงข้อมูล
2. Database Container ทำหน้าที่จัดเก็บข้อมูล รองรับการเพิ่ม แก้ไข และดึงข้อมูลจาก Web Application แยกการทำงานออกจากตัวแอปเพื่อความเป็นระบบและปลอดภัย
```
...
คำตอบ:
```
Volume ถูกเชื่อมกับ Database Container เพื่อเก็บข้อมูลถาวร
ข้อดี: ข้อมูลไม่หายแม้ Container ถูกลบหรือสร้างใหม่
ข้อเสีย: ไม่สามารถดูไฟล์ได้โดยตรงจากโฟลเดอร์โปรเจกต์
```
...
คำตอบ:
```
Network ใน Docker Compose ทำหน้าที่เป็นเครือข่ายเสมือน (Virtual Network) ที่เชื่อมต่อ Container ต่าง ๆ ให้สามารถสื่อสารกันได้อย่างปลอดภัยภายในระบบเดียวกัน
Container สามารถสื่อสารกันได้โดย ใช้ ชื่อ service ที่กำหนดไว้ในไฟล์ docker-compose.yml แทนการใช้ IP Address
```
...
คำตอบ:
```
ป้องกันปัญหาแอปพลิเคชันเริ่มทำงานก่อนบริการที่จำเป็น ช่วยให้การรันระบบด้วย docker-compose up เป็นไปอย่างเป็นลำดับ
```
...
คำตอบ:
```
ต้องแก้ไขที่ส่วน ports ของ service ที่รัน Moodle ในไฟล์ docker-compose.yml
```
...
คำตอบ:
```
MOODLE_DB_HOST=db หมายความว่า กำหนดให้ Moodle เชื่อมต่อไปยังฐานข้อมูลที่อยู่ใน Container ชื่อ db ซึ่งเป็นชื่อ service ของ Database ที่ประกาศไว้ในไฟล์ docker-compose.yml
เหตุผลที่ ไม่ใช้ localhost เพราะว่า Moodle และ Database อยู่คนละ Container
```
...
คำตอบ:
```
การติดตั้ง Moodle ด้วย Docker
ข้อดี:  ติดตั้งง่ายและรวดเร็ว
      ไม่กระทบระบบหลักของเครื่อง
      ย้ายเครื่องหรือแชร์โปรเจกต์ได้สะดวก
      สภาพแวดล้อมเหมือนกันทุกเครื่อง
ข้อเสีย: ต้องมีความรู้ Docker เพิ่มเติม
การดีบักอาจซับซ้อนกว่าการติดตั้งปกติ
ใช้ทรัพยากรระบบมากขึ้นเล็กน้อย

การติดตั้งแบบปกติ (Manual Install)
ข้อดี:เข้าใจโครงสร้างระบบชัดเจน
เหมาะกับเซิร์ฟเวอร์จริงระยะยาว
ควบคุมระบบได้ละเอียด
ข้อเสีย:ขั้นตอนติดตั้งซับซ้อน
เสี่ยงต่อปัญหาความเข้ากันของเวอร์ชัน
ย้ายหรือสำรองระบบทำได้ยากกว่า
```
...
คำตอบ:
```
วิธีตรวจสอบ: docker-compose logs moodle_app
